...
module Spree
  class VendorAbility
    ...
    def initialize(user)
      ...
      return unless user.is_a?(Spree.admin_user_class)
      return unless user.try(:vendors)

      @vendor_ids = user.vendors.ids

      return if @vendor_ids.empty?

      @current_vendor = user.vendors.with_access_to_ui.first
      ...
      if @vendor_ids.any?
        ...
        apply_webhook_subscribers_permissions
        apply_digital_permissions
        apply_taxons_permissions
        apply_shop_permission
        apply_webhook_subscribers_permissions
    ...
    def apply_shop_permission
      return unless defined?(Spree::ExternalStore)

      can :create, Spree::ExternalStore
    end

    def apply_webhook_subscribers_permissions
      can :create, Spree::Webhooks::Subscriber
    end
    ...
    def apply_product_permissions
      ...
      can :manage_option_types, Spree::Product, vendor_id: @vendor_ids, external_product_id: nil
      cannot :create, Spree::Product
      can :create, Spree::Product unless @current_vendor.present? && @current_vendor.respond_to?(:external_store) && @current_vendor.external_store.present?
    ...
    def apply_payouts_permissions
      return unless defined?(SpreeStripe::Gateway)
      return unless defined?(SpreeStripe::Transfer)

      can :admin, :payouts
      can :index, :payouts
    end

    def apply_stripe_transfers_permissions
      return unless defined?(SpreeStripe::Gateway)
      return unless defined?(SpreeStripe::Transfer)

      can :admin, SpreeStripe::Transfer
      can :index, SpreeStripe::Transfer
    end
    ...
    def apply_webhook_subscribers_permissions
      can :create, Spree::Webhooks::Subscriber
      can :manage, Spree::Webhooks::Subscriber, vendor_id: @vendor_ids
    end
