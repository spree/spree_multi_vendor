shared_context 'creates vendor order with line item' do
  let!(:line_item) { create(:vendor_line_item, order: order, currency: currency, vendor: vendor) }
...
shared_context 'vendor order with a physical line item' do
  include_context 'creates vendor order with line item'
end

shared_context 'vendor order with a digital line item' do
  let!(:digital) { create(:digital, variant: create(:product_in_stock, vendor: vendor).default_variant) }
  let!(:variant_digital) { digital.variant }
  let!(:line_item) { create(:vendor_line_item, variant: variant_digital, order: order, currency: currency) }
  let!(:headers) { headers_bearer }
...
shared_context 'vendor order with a physical and digital line item' do
  let!(:digital) { create(:digital, variant: create(:product_in_stock, vendor: vendor).default_variant) }
  let!(:variant_digital) { digital.variant }
  let!(:digital_line_item) { create(:vendor_line_item, vendor: vendor, variant: variant_digital, order: order, currency: currency) }
  let!(:physical_line_item) { create(:vendor_line_item, vendor: vendor, order: order, currency: currency) }
...
shared_context 'creates vendor guest order with guest token' do
